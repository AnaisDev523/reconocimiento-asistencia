...
IMG_W = 100
IMG_H = 100
CAPTURE_COUNT = 15  # número de imágenes por usuario
...
def ensure_user_folder(uid: int) -> str:
    folder = os.path.join(DATA_DIR, str(uid))
    os.makedirs(folder, exist_ok=True)
    return folder


def preprocess_face_from_bgr(img_bgr: np.ndarray) -> np.ndarray:
    gray = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2GRAY)
    face = cv2.resize(gray, (IMG_W, IMG_H))
    return face.flatten()


def detect_and_crop(image_bgr: np.ndarray):
...
@app.route("/")
def index():
    # igual que tu idea original, pero verificando si el modelo ya existe
...
def register():
    if request.method == "POST":
        ...
        cur.execute(
            "INSERT INTO usuarios (nombre, carpeta, fecha_registro) VALUES (?, ?, ?)",
            (name, "", fecha),
        )
        uid = cur.lastrowid
        carpeta = str(uid)
...
# --------- NUEVO: detección rápida para los bordes ----------


@app.route("/detect_temp_face", methods=["POST"])
def detect_temp_face():
    """
    Analiza un frame de la cámara y devuelve:
      - "good"  -> rostro bien encuadrado
      - "weak"  -> rostro detectado pero podría estar mejor
...
def train():
    ...
    for row in rows:
        ...
        for fname in os.listdir(folder):
            ...
            if cropped is None:
                face = cv2.resize(
                    cv2.cvtColor(img, cv2.COLOR_BGR2GRAY), (IMG_W, IMG_H)
                )
            else:
                face = cv2.resize(
                    cv2.cvtColor(cropped, cv2.COLOR_BGR2GRAY), (IMG_W, IMG_H)
                )
    ...
    if len(X) == 0:
        return (
            jsonify({"success": False, "error": "No hay imágenes para entrenar"}),
            400,
        )
...
def recognize():
    ...
    neigh_dist, neigh_idx = knn.kneighbors(
        vec,
        n_neighbors=min(3, len(knn._fit_X)),
    ...
    if label is None:
        row = conn.execute(
            "SELECT nombre FROM usuarios WHERE id = ?", (pred,)
        ).fetchone()
        if row:
            label = row["nombre"]
            conn.execute(
                "INSERT INTO accesos (usuario_id, nombre, fecha_hora) VALUES (?, ?, ?)",
                (
                    int(pred),
                    label,
...
def logs():
    conn = get_db_connection()
    rows = conn.execute(
        "SELECT id, usuario_id, nombre, fecha_hora "
        "FROM accesos ORDER BY fecha_hora DESC"
    ).fetchall()
    conn.close()
...
def users():
    conn = get_db_connection()
    rows = conn.execute(
        "SELECT id, nombre, carpeta, fecha_registro FROM usuarios"
    ).fetchall()
    conn.close()
...
def export_pdf():
    ...
    pdf.cell(200, 10, "Registro de Accesos", ln=True, align="C")
    pdf.ln(5)

    pdf.set_font("Arial", style="B", size=10)
    pdf.cell(15, 8, "ID", 1)
    pdf.cell(25, 8, "User ID", 1)
    pdf.cell(60, 8, "Nombre", 1)
    pdf.cell(60, 8, "Fecha/Hora", 1)
    pdf.ln()

    pdf.set_font("Arial", size=9)
    for _, row in df.iterrows():
        pdf.cell(15, 8, str(row["id"]), 1)
        pdf.cell(25, 8, str(row["usuario_id"]), 1)
        pdf.cell(60, 8, str(row["nombre"]), 1)
        pdf.cell(60, 8, str(row["fecha_hora"]), 1)
        pdf.ln()

    file_path = "accesos.pdf"
